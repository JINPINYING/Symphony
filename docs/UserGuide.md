...
Dashboard and `/api/v1/state` token totals are derived from absolute Codex usage payloads such as `thread/tokenUsage/updated` and `total_token_usage`. Delta-only payloads like `last_token_usage` are ignored so counters do not stall at zero or double-count.
